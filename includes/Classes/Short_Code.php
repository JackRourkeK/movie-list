...
class Short_Code {
	...
	public static function generate_shortcode() {
		add_shortcode( 'bu_movie_list', array( self::class, 'generate_html_shortcode' ) );
	}

	/**
	 * Function to generate html and pagination when implementing the shortcode
	 *
	 * @param mixed $shortcode_parameters (Parameter to define the pagination implementation for the post).
	 */
	public static function generate_html_shortcode( $shortcode_parameters = null ) {
		...
		foreach ( $get_movie_list as $movie_list ) {
			...
			if ( ! empty( $get_movie_tags ) ) {
				$movie_tags .= '<br><strong>' . esc_html__( 'Movie Tags', 'movie-list' ) . ': </strong>' . $get_movie_tags_name;
			}
